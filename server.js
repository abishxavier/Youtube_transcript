...
/**
 * Extract a JSON array from HTML by key using balanced bracket counting.
 * Handles nested objects, escaped quotes, and large polymer watch pages without regex truncation.
 */
function extractJsonArray(html, key) {
  ...
  const start = html.indexOf('[', idx + token.length);
...
async function fetchCaptionsTrack(videoId) {
  // 1. Primary: InnerTube API (Android client context)
  const innerTracks = await fetchInnerTubeCaptionTracks(videoId);
  if (innerTracks && innerTracks.length > 0) {
    return innerTracks;
  }

  // 2. Secondary: Watch page scrape using balanced bracket JSON extraction
  try {
    ...
    const captionTracks = extractJsonArray(html, 'captionTracks');
    if (Array.isArray(captionTracks) && captionTracks.length > 0) {
      return captionTracks;
    }
  } catch (err) {
...
async function fetchTimedText(baseUrl) {
  try {
    const finalUrl = baseUrl.includes('&fmt=') ? baseUrl : `${baseUrl}&fmt=srv3`;
    const res = await fetchYouTube(finalUrl, {
      headers: {
  ...
  try {
      ...
      for (const tLang of tryLangs) {
        try {
          const raw = await YoutubeTranscript.fetchTranscript(videoId, {
            lang: tLang,
            fetch: (url, opts = {}) => fetchYouTube(url, { ...opts, signal: AbortSignal.timeout(5000) }),
          });
          if (raw && raw.length > 0) {
    ...
    if (!transcript || transcript.length === 0) {
      ...
      try {
        ...
        // If caption tracks exist from previous steps, return them for client-side fetch instead of failing
        if (captionTracks && captionTracks.length > 0) {
          const targetTrack = (requestedLang ? captionTracks.find(t => t.languageCode === requestedLang) : null) || captionTracks[0];
          const rawUrl = targetTrack.baseUrl || '';
          ...
          return res.json({
            ...
            fallbackUrl,
            sourceLanguage: detectedSourceLang || targetTrack.languageCode || 'en',
            language: requestedLang || targetTrack.languageCode || 'en',
            availableTracks: captionTracks.map(t => ({
              name: t.name?.runs?.[0]?.text || t.name?.simpleText || t.languageCode,
              languageCode: t.languageCode,
              baseUrl: t.baseUrl ? (t.baseUrl.includes('&fmt=') ? t.baseUrl : `${t.baseUrl}&fmt=srv3`) : null,
